import type { HashAlgorithm } from '@/utils/hashing';

import { generateHash } from '@/utils/hashing';
import { describe, expect, it } from 'vitest';

describe('Hashing', () => {
    describe('generateHash', () => {
        // should probably add more cases per algorithm but this is fine
        const hashTests: [HashAlgorithm, string][] = [
            [
                'SHA-1',
                '2ef7bde608ce5404e97d5f042f95f89f1c232871',
            ],
            [
                'SHA-256',
                '7f83b1657ff1fc53b92dc18148a1d65dfc2d4b1fa3d677284addd200126d9069',
            ],
            [
                'SHA-384',
                'bfd76c0ebbd006fee583410547c1887b0292be76d582d96c242d2a792723e3fd6fd061f9d5cfd13b8f961358e6adba4a',
            ],
            [
                'SHA-512',
                '861844d6704e8573fec34d967e20bcfef3d424cf48be04e6dc08f2bd58c729743371015ead891cc3cf1c9d34b49264b510751b1ff9e537937bc46b5d6ff4ecc8',
            ],
            ['CRC-32', '1c291ca3'],
        ];

        it.each(hashTests)('should hash %s', async (algorithm, expected) => {
            // Arrange
            const input = new TextEncoder().encode('Hello World!');

            // Act
            const hash = await generateHash(algorithm, input);
            const hashStr = hash.toHex();

            // Assert
            expect(hashStr).toBe(expected);
        });
    });
});
